...
export default function EditDestinationPage() {
  ...
  const { destinations, categories, updateDestination } = useApp();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (params?.id) {
      updateDestination(params.id as string, {
        name,
        category,
        description,
        address,
  ...
  return (
    <div className="flex flex-col gap-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-2">
        <Link href={`/destinations/${params.id}`} className="text-gray-500 hover:text-[#0F172A] transition-colors p-1.5 rounded-lg hover:bg-gray-100">
          <ArrowLeft size={16} />
        </Link>
        <span className="text-xs text-gray-400 font-bold">Kembali ke Detail Destinasi</span>
      </div>

      {/* Header */}
      <div className="flex flex-col gap-1.5">
        <h1 className="text-2xl font-bold text-[#0F172A] tracking-tight">Ubah Destinasi</h1>
        <p className="text-xs text-gray-400 font-semibold">
          Perbarui informasi untuk objek wisata <strong>{name}</strong>.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col gap-5">
          ...
          <Textarea
            ...
            placeholder="Jelaskan daya tarik destinasi..."
            value={description}
            onChange={(e) => {
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col gap-5">
          ...
          <Textarea
            ...
            placeholder="Nama jalan, kecamatan, Lampung..."
            rows={2}
            value={address}
            onChange={(e) => {
